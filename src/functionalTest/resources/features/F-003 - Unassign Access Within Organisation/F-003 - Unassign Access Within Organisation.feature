...
Feature: F-003: Unassign Access Within Organisation
  ...
  Scenario: Solicitor successfully removing case access for another solicitor in their org (happy path)
    ...
    And a case [by U1 to create a case - C1] created as in [F-003_Prerequisite_Case_Creation_C1],
    And a wait time of [5] seconds [to allow for the case just created to appear in search results],
    And a successful call [by U1 to grant access to C1 for U2] as in [Prerequisite_Case_Assignment_C1_U2],
    And a successful call [by U1 to grant access to C1 for U3] as in [Prerequisite_Case_Assignment_C1_U3],
    And a successful call [by U1 to confirm the access to C1 for U2 & U3] as in [Prerequisite_Case_Access_Confirmation_C1_U2_U3],
  ...
  Scenario: Solicitor successfully removing access to multiple cases for multiple solicitors in their org (happy path)
    ...
    And a case [by U1 to create a case - C1] created as in [F-003_Prerequisite_Case_Creation_C1],
    And a case [by U1 to create a case - C2] created as in [F-003_Prerequisite_Case_Creation_C2],
    And a case [by U1 to create a case - C3] created as in [F-003_Prerequisite_Case_Creation_C3],
    And a wait time of [5] seconds [to allow for the case just created to appear in search results],
    And a successful call [by U1 to grant access to C1 for U2] as in [Prerequisite_Case_Assignment_C1_U2],
    And a successful call [by U1 to grant access to C1 for U3] as in [Prerequisite_Case_Assignment_C1_U3],
    And a successful call [by U1 to grant access to C2 for U2] as in [Prerequisite_Case_Assignment_C2_U2],
    And a successful call [by U1 to grant access to C2 for U3] as in [Prerequisite_Case_Assignment_C2_U3],
    And a successful call [by U1 to grant access to C3 for U2] as in [Prerequisite_Case_Assignment_C3_U2],
    And a successful call [by U1 to grant access to C3 for U3] as in [Prerequisite_Case_Assignment_C3_U3],
    And a successful call [by U1 to confirm the access to C1, C2 & C3 for U2 & U3] as in [Prerequisite_Case_Access_Confirmation_C1_C2_C3_U2_U3],
    When a request is prepared with appropriate values,
  ...
  Scenario: Pui-caa successfully removing access to multiple cases for multiple solicitors in their org (happy path)
    ...
    And a successful call [by U2 to confirm the access to C1, C2 & C3 for U3] as in [Prerequisite_Case_Access_Confirmation_C1_C2_C3_U3],
    When a request is prepared with appropriate values,
  ...
  Scenario: Must return an error response if the invoker doesn't have access to the case of the user they are trying to remove access for
    ...
    And a case [by U1 to create a case - C1] created as in [F-003_Prerequisite_Case_Creation_C1],
    And a wait time of [5] seconds [to allow for the case just created to appear in search results],
    And a successful call [by U1 to confirm the access to C1 for U1] as in [Prerequisite_Case_Access_Confirmation_C1_U1],
    When a request is prepared with appropriate values,
    And the request [is made by U2 and intends to unassign access to C1 for U1],
    And it is submitted to call the [Unassign Access within Organisation] operation of [Manage Case Assignment Microservice],
    Then a negative response is received,
