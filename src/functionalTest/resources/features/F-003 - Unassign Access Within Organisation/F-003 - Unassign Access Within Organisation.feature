...
Scenario: Solicitor successfully removing case access for another solicitor in their org (happy path)
    ...
    Given a user [Becky - with a Solicitor role for a particular jurisdiction under an organisation to create, assign and unassign access to a case for another solicitor in their organisation],
      ...
      And a successful call [by Becky to grant access to C1 for Benjamin] as in [Prerequisite_Case_Assignment_C1_Benjamin],
      And a successful call [by Becky to grant access to C1 for Bill] as in [Prerequisite_Case_Assignment_C1_Bill],
      And a successful call [by Becky to confirm the access to C1 for Benjamin & Bill] as in [Prerequisite_Case_Access_Confirmation_C1_Benjamin_Bill_By_Becky],
...
Scenario: CAA successfully removing case access for another solicitor in their org (happy path)
    ...
    Given a user [Becky - with a solicitor role in a particular jurisdiction within an organisation, to create a case],
      ...
      And a case [by Becky to create a case - C1] created as in [F-003_Prerequisite_Case_Creation_C1],
      And a wait time of [5] seconds [to allow for the case just created to appear in search results],
      And a successful call [by CAA to grant access to C1 for Becky] as in [Prerequisite_Case_Assignment_C1_Becky],
      And a successful call [by CAA to grant access to C1 for Bill] as in [Prerequisite_Case_Assignment_C1_Bill_By_CAA],
      And a successful call [by CAA to confirm the access to C1 for Becky & Bill] as in [Prerequisite_Case_Access_Confirmation_C1_Becky_Bill_By_CAA],
...
Scenario: Solicitor successfully removing access to multiple cases for multiple solicitors in their org (happy path)
    ...
    Given a user [Becky - with a Solicitor role in a jurisdiction under an organisation to assign and Unassign a case role to a solicitor within the same organisation],
      ...
      And a successful call [by Becky to grant access to C1 for Benjamin] as in [Prerequisite_Case_Assignment_C1_Benjamin],
      And a successful call [by Becky to grant access to C1 for Bill] as in [Prerequisite_Case_Assignment_C1_Bill],
      And a successful call [by Becky to grant access to C2 for Benjamin] as in [Prerequisite_Case_Assignment_C2_Benjamin],
      And a successful call [by Becky to grant access to C2 for Bill] as in [Prerequisite_Case_Assignment_C2_Bill],
      And a successful call [by Becky to grant access to C3 for Benjamin] as in [Prerequisite_Case_Assignment_C3_Benjamin],
      And a successful call [by Becky to grant access to C3 for Bill] as in [Prerequisite_Case_Assignment_C3_Bill],
      And a successful call [by Becky to confirm the access to C1, C2 & C3 for Benjamin & Bill] as in [Prerequisite_Case_Access_Confirmation_C1_C2_C3_Benjamin_Bill_By_Becky],
...
Scenario: Pui-caa successfully removing access to multiple cases for multiple solicitors in their org (happy path)
    ...
    Given a user [Becky - with a solicitor role in a particular jurisdiction within an organisation, to create a case],
      ...
      And a wait time of [5] seconds [to allow for the case just created to appear in search results],
      And a successful call [by CAA to grant access to C1 for Bill] as in [Prerequisite_Case_Assignment_C1_Bill_By_CAA],
      And a successful call [by CAA grant access to C2 for Bill] as in [Prerequisite_Case_Assignment_C2_Bill_By_CAA],
      And a successful call [by CAA to grant access to C3 for Bill] as in [Prerequisite_Case_Assignment_C3_Bill_By_CAA],
      And a successful call [by CAA to confirm the access to C1, C2 & C3 for Bill] as in [Prerequisite_Case_Access_Confirmation_C1_C2_C3_Bill_By_CAA],
...
Scenario: Must return an error response if the invoker is a solicitor in a different jurisdiction from that of the case
    ...
    Given a user [Becky - with a Solicitor role for a particular jurisdiction under an organisation to create, assign and unassign access to a case for another solicitor in their organisation],
      ...
      And a case [by Becky to create a case - C1] created as in [F-003_Prerequisite_Case_Creation_C1],
      And a wait time of [5] seconds [to allow for the case just created to appear in search results],
      And a successful call [by Becky to confirm the access to C1 for Becky] as in [Prerequisite_Case_Access_Confirmation_C1_Becky_By_Becky],
