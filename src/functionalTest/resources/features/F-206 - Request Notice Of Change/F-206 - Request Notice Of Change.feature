...
# ACA-68 / AC-6
@S-206.6
Scenario: (Happy Path) CAA (also a solicitor for a different jurisdiction) requests NoC for a non-represented litigant - no auto-approval applies
...
Scenario: (Happy Path) Solicitor requests NoC to replace representation - auto-approval applies
    ...
    Given a user [Richard - with the ability to create a case for a particular jurisdiction within an organisation],
      ...
      And a successful call [by Richard to create a case C1, auto-assigned to his organisation, containing 3 Org Policies for 3 case roles: R1, R2 & R3] as in [Prerequisite Case Creation Call for Case Assignment],
      And a successful call [by Matt to place an NoC Request which contains answers identifying case role R1],
...
Scenario: (Happy Path) Solicitor requests NoC for a non-represented litigant - auto-approval applies
    ...
    Given a user [Mario - to initiate a case on his own behalf via a citizen facing application]
      ...
      And a successful call [by Mario to create a case C1 with 3 Org Policies for 3 case roles: R1, R2 & R3  - via a citizen-facing application] as in [Prerequisite Case Creation Call for Case Assignment],
      And a successful call [by Matt to place an NoC Request which contains answers identifying case role R1],
...
# ACA-68 / AC-13
@S-206.13
Scenario: (Happy Path) CAA (also a solicitor for the same jurisdiction) requests NoC to replace representation - auto-approval applies - TBC with Nigel

    Given a user [Richard - with the ability to create a case for a particular jurisdiction within an organisation],
      And a user [Jane - with a pui-caa role in Richard's organisation, also a solicitor for the jurisdiction of a Case C1 to be created by Richard],
      And a user [Matt - with a pui-caa role for a different organisation to that of Richard],
      And a successful call [by Richard to create a case C1, auto-assigned to his organisation, containing 3 Org Policies for 3 case roles: R1, R2 & R3] as in [Prerequisite Case Creation Call for Case Assignment],
      And a successful call [by Matt to trigger a NoC Request which contains answers identifying case role R1],
...
# ACA-68 / AC-14
@S-206.14
Scenario: (Happy Path) CAA (also a solicitor for the same jurisdiction) requests NoC for a non-represented litigant - auto-approval applies - TBC with Nigel

    Given a user [Mario - to initiate a case on his own behalf via a citizen facing application]
      And a user [Jane - as a CAA in an organisation with also a solicitor role for the jurisdiction of a Case C1 to be created by Mario],
      And a user [Matt - as another CAA in Jane's organisation],
      And a successful call [by Mario to create a case C1 containing 3 Org Policies for 3 case roles: R1, R2 & R3 - via a citizen facing application] as in [Prerequisite Case Creation Call for Case Assignment],
      And a successful call [by Matt to trigger a NoC Request which contains answers identifying case role R1],
