...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# CCD-5334 / AC02 Similar to S-206.09 but with a different user (MATT) requesting the NoC With Specific Access
@Ignore @S-206a.01 @callbackTests
Scenario: (Happy Path) CAA requests NoC to replace representation - auto-approval applies
    ...
    Given a user [Richard - with the ability to create a case for a particular jurisdiction within an organisation],
     ...
     Then a positive response is received,
      ...
      And a call [to get Grant Access Metadata API returning Specific Grant Access for case] will get the expected response as in [F-206_Verify_NoC_Request_Specific_Access_MetadataGrantNone]
...
# CCD-5334 / AC-02 similar to @S-206.13 but with a different user (JANE) requesting the NoC With Specific Access
  ...
  Scenario: (Happy Path) CAA (also a solicitor for the same jurisdiction) requests NoC to replace representation With Specific Access - auto-approval applies
    ...
    And a call [to get Grant Access Metadata API returning Standard Grant Access for case] will get the expected response as in [F-206_Verify_NoC_Request_Access_Metadata_Jane]
